  ...
  return (
    <div className="p-6 max-w-7xl mx-auto" dir="rtl">
      <header className="mb-6">
        ...
        <p className="text-slate-500 mt-1">قبول/رفض طلبات السحب — يتم خصم الرصيد تلقائياً عند القبول</p>
      </header>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50/50">
          <span className="text-xs text-slate-500">{data?.total ?? 0} طلب · صفحة {page} من {totalPages}</span>
...
function WithdrawalRow({ w }: { w: SuperWithdrawal }) {
  ...
  const [actionOpen, setActionOpen] = useState<null | "approve" | "reject">(null);
    ...
    <>
      <tr className="hover:bg-slate-50">
        ...
        <td className="px-4 py-2">
          ...
          <div className="font-mono text-[10px] text-slate-400" dir="ltr">{w.userTelegramId ? String(w.userTelegramId) : ""}</div>
        </td>
        <td className="px-4 py-2 font-mono" dir="ltr">{Number(w.amount).toFixed(4)} <span className="text-xs uppercase text-slate-500">{w.currency}</span></td>
        ...
        <tr className="bg-emerald-50/50">
          <td colSpan={8} className="px-4 py-3">
            <div className="flex items-center gap-2">
              <input value={txHash} onChange={(e) => setTxHash(e.target.value)} placeholder="Transaction hash (اختياري)" dir="ltr"
      ...
      {actionOpen === "reject" && (
        <tr className="bg-red-50/50">
          <td colSpan={8} className="px-4 py-3">
            <div className="flex items-center gap-2">
              <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="سبب الرفض"
                className="flex-1 px-3 py-2 rounded-lg border border-red-200 text-sm" />
              <button onClick={() => rejectMut.mutate()} disabled={rejectMut.isPending || !reason.trim()}
                className="px-3 py-2 text-sm bg-red-600 hover:bg-red-500 text-white rounded font-semibold disabled:opacity-40">
                {rejectMut.isPending ? "…" : "تأكيد الرفض"}
              </button>
            </div>
            {err && <div className="text-red-600 text-sm mt-1">{err}</div>}
          </td>
